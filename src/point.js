...
async function renderQueue() {
  ...
  queueOutput.textContent = events.length ? events.map(event => `${event.occurredAt} — ${event.status} — ${event.clientEventId}${event.status === 'REJECTED' ? ` — ${event.rejectionReason}` : ''}`).join('\n') : 'Nenhuma marcação pendente.';
}
